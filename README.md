...
- 範例檔下載：人員與獎項頁都可下載真正的 XLSX 範例檔，用 Excel 編輯後再複製內容貼回系統。
...
- 清除所有暫存資料：刪除活動名稱、人員、獎項、中獎結果與所有抽獎進度，讓系統回到初始狀態。

## 貼上匯入格式

系統仍採用貼上匯入，不直接上傳 Excel 檔。可先下載 `.xlsx` 範例檔，用 Excel 編輯完成後，複製表格內容並貼到匯入欄位。
